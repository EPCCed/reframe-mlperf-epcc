...
gc = GlobalContext("/work/ta127/ta127/chrisrae/chris-ml-intern/ML/ResNet50/Torch/config.yaml")
...
def main():
    ...
    if gc["opt"]["name"].upper() == "SGD":
        ...
        opt = LARS(model.parameters(), lr=gc["lr_schedule"]["base_lr"], momentum=gc["opt"]["momentum"], weight_decay=gc["opt"]["weight_decay"])
    else:
        raise ValueError(f"Optimiser {gc['opt']['name']} not supported please use SGD|LARS")
    
    scheduler = torch.optim.lr_scheduler.PolynomialLR(opt, total_iters=gc["data"]["n_epochs"], power=gc["lr_schedule"]["poly_power"])
    
    loss_fn = torch.nn.CrossEntropyLoss()

    train_metric = torchmetrics.classification.Accuracy(task="multiclass", num_classes=1000)
    val_metric = torchmetrics.classification.Accuracy(task="multiclass", num_classes=1000)
