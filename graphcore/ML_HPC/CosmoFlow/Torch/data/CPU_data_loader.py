from itertools import cycle  # correct batch sizing
import os
...
class CosmoData:
    def __init__(self, file_path, train=True, batch_size=64, shuffle=False):
        ...
        self.file_path = os.path.join(file_path, "train/" if train else "val/")
        self.files = cycle(
            chunk_files_per_worker(os.listdir(self.file_path), world_size)[rank]
        )
        ...
        (
            ...
            self.labels[: self.buffersize],
        ) = self._process_file()
        (
            self.data[self.buffersize:],
            self.labels[self.buffersize:],
        ) = self._process_file()  # buffer
    ...
    def _read_file(self, file):
        ...
        # not memory effecient
        return file_out["data"], file_out["label"].repeat(64, 1)
